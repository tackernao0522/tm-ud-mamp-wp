...
class WelcomePage {
    ...
    public function activated_plugin($plugin) {
        ...
        if (!$fromBulkAction && !$isWpCli && $plugin === plugin_basename($this->getInitiator()->getPluginFile())) {
            $redirectState = \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $this->getInitiator(),
                \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_NOT_REGISTERED
            );
            if (
                $redirectState ===
                \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_NOT_REGISTERED
            ) {
                \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::set(
                    $this->getInitiator(),
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_AWAITING_REDIRECT
                );
            }
        }
    }
    ...
    public function admin_head_probably_redirect() {
        if (
            \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\Core::getInstance()
                ->getAssets()
            ...
            if (
                ...
                \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::set(
                    $this->getInitiator(),
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_REDIRECTED
                );
    ...
    <div class="about-wrap-content">
        ...
        $data = [
            ...
            '%subtitle' => \str_replace(
                ...
                \sprintf(
                    '<br /><cite>',
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\Core::getInstance()->getBaseAssetsUrl(
                        'devowl.io-logo.svg'
                    )
                ),
        ...
        return is_admin() &&
            ...
            $_GET['page'] ===
                $this->getInitiator()->getPluginSlug() .
                    \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\WelcomePage::PAGE_SUFFIX;
    }
    /**
     * Create instance.
     *
    ...
    public static function instance($initiator) {
        return new \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\WelcomePage($initiator);
    }
}
