...
' .
    ...
    public function routeCrossDismiss($request) {
        ...
        $impl = \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSelling($slug);
        if ($impl === null) {
            return new \WP_Error(
                'rest_error',
    ...
    public function routeRatingDismiss($request) {
        $slug = $request->get_param('slug');
        $force = $request->get_param('force');
        $result = \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\Core::getInstance()
            ->getRatingHandler()
            ->dismiss($slug, $force);
        // Currently, ignore errors
        return new \WP_REST_Response(['result' => $result]);
    ...
    public static function instance() {
        return new \DevOwl\RealThumbnailGenerator\Vendor\DevOwl\RealUtils\Service();
    }
}
