...
## 116. Twenty Twenty-Oneでブログを構築しよう①

+ http://localhost/blog/wp-admin/install.php (WordPressセットアップ画面) => `サイトのタイトル` => `たかちゃんブログ。`と入力<br>

+ `ユーザー名` => `takachan_admin_5573`と入力 => `パスワード` => `OJfndtGclP0m3a#f7r` => `メールアドレス` => `takaproject777@gmail.com`と入力してみる => `検索エンジンでの表示` => 開発環境の場合はどちらでのよい => `WordPress をインストール`<br>

+ 成功しました！と出るので`ログイン`する => `ユーザー名` => `takachan_admin_5573` => `パスワード` => `OJfndtGclP0m3a#f7r`<br>

+ wp管理画面 => `外観` => `テーマ` => `Twenty Twenty-One`を有効化しておく<br>

+ サイトを表示して確認してみる<br>

+ wp管理画面 => `設定` => `一般` => `キャッチフレーズ` => `案件ください。一生懸命勉強してます。`と入れてみる => `変更を保存` => サイトを更新して確認してみる <br>

+ wp管理画面 => `設定` => `投稿設定` => 特に変更箇所はなし<br>

+ wp管理画面 => `設定` => `表示設定` => `1ページに表示する最大投稿数` => `20`にしておく => `RSS/Atom フィードで表示する最新の投稿数 => `20`にしておく => `変更を保存` <br>

+ wp管理画面 => `設定` => `ディスカッション` => `デフォルトの投稿設定` => `上の2つのチェックを外しておく` => 以降は好みに応じて変更する => `変更を保存`<br>

+ wp管理画面 => `設定` => `メディア` => 特にそのままで良い<br>

+ wp管理画面 => `設定` => `パーマリンク` => `月と投稿名`を選択して => `カスタム構造` => `/%year%/%monthnum%/%post_id%/`と変更する(%postname% の箇所を%post_id%に変更) => `変更を保存`<br>

+ wp管理画面 => `設定` => `プライバシー` => 特に変更なし<br>

+ サイトを更新して確認してみる<br>

## 117. Twenty Twenty-Oneでブログを構築しよう②

+ wp管理画面 => `プラグイン` => `インストール済みプラグイン` => 既にプラグインは削除していないので入っているので必要なプラグインを有効化する => (Edit Author Slug, SEO SIMPLE PACK, SVG Support)の３つをとりあえず有効化しておく<br>

+ 使いいないプラグインは削除しておいた方が良いので => `停止中` => 全てにチェックを入れて => `一括操作` => `削除` => `適用` => `OK`<br>
